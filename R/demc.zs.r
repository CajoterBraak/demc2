...
for (iter in 1:n.generation) {
   accepti = 0
     for (i in chainset){
       ...
       if ( runif(1)< pSnooker ) {  
       # DE-Snooker update
         # if (Npop >1) { z =  X[,sample(chainset[-i],1)]} else { # no real advantage and precludes parallel computing
 ...
 list(Draws= Z[,-(1:(M0 + Npop* floor(n.burnin/n.thin))),drop = FALSE] , accept.prob.mat= Naccept/n.generation, X.final = X, logfitness.X.final = logfitness_X, Npop=Npop)
}
